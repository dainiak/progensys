...
from blueprints.models import db, Role, Problem, Participant, ProblemSetExtra, ProblemSet, Course, ProblemSetContent
...
def api():
    ...
    if action == "load":
        problems = (
            db.session.query(ProblemSetContent.sort_key, Problem.id, Problem.statement)
            .filter(ProblemSetContent.problem_set_id == problem_set_id, ProblemSetContent.problem_id == Problem.id)
            .all()
        )
        extras = (
            db.session.query(ProblemSetExtra.sort_key, ProblemSetExtra.id, ProblemSetExtra.content)
        ...
        if item.get("problem_set_extra_id"):
            db.session.delete(ProblemSetExtra.query.filter_by(id=item["problem_set_extra_id"]).first())
        elif item.get("problem_id"):
            db.session.delete(
                ProblemSetContent.query.filter_by(problem_set_id=problem_set_id, problem_id=item["problem_id"]).first()
            )
        else:
            abort(400)
        db.session.commit()
        ...
        if item.get("problem_id"):
            ...
            if (
                problem
                and not db.session.query(ProblemSetContent.problem_id)
                .filter(ProblemSetContent.problem_id == problem_id, ProblemSetContent.problem_set_id == problem_set_id)
                .first()
            ):
        ...
        for i, item in enumerate(items):
            ...
            if problem_id:
                ProblemSetContent.query.filter_by(
                    problem_id=problem_id, problem_set_id=problem_set_id
                ).first().sort_key = 10 * (i + 1)
            else:
                ProblemSetExtra.query.filter_by(id=problem_set_extra_id).first().sort_key = 10 * (i + 1)
        db.session.commit()
        return jsonify(result="Success")
