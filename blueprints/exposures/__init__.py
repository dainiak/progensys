from flask import Blueprint, render_template, request, abort, jsonify, redirect, url_for, json as flask_json
import flask_login
...
def view_exposure(course_id, exposure_string):
    if (
        not db.session.query(Participant)
        .filter(
    ...
    if "-" in exposure_string:
        ...
        exposure_ids = list(
            map(attrgetter("id"), filter(lambda x: x.timestamp.isoformat()[:10] == exposure_string, exposures))
        )
        if exposure_ids:
...
def print_exposure(course_id, exposure_string):
    ...
    results = []
    for firstname, lastname, user_id, problem_set_id, problem_set_is_adhoc in (
        db.session.query(User.name_first, User.name_last, User.id, ProblemSet.id, ProblemSet.is_adhoc)
        .filter(
            ExposureContent.exposure_id == exposure_id,
            ExposureContent.user_id == User.id,
            ProblemSet.id == ExposureContent.problem_set_id,
        )
        .order_by(ExposureContent.sort_key)
        .all()
    ):
        groups_string = ", ".join(
            x[0]
            for x in db.session.query(Group.code)
            .filter(Group.id == GroupMembership.group_id, GroupMembership.user_id == user_id)
            .all()
        )
        results.append(
            {
        ...
        for problem_id, problem_statement, sort_key in (
            db.session.query(Problem.id, Problem.statement, ProblemSetContent.sort_key)
            .filter(Problem.id == ProblemSetContent.problem_id, ProblemSetContent.problem_set_id == problem_set_id)
            .all()
        ):
            problem_level = (
                db.session.query(TopicLevelAssignment.level)
                .filter(
        ...
        for sort_key, extra_content in (
            db.session.query(ProblemSetExtra.sort_key, ProblemSetExtra.content)
            .filter(ProblemSetExtra.problem_set_id == problem_set_id)
            .all()
        ):
            results[-1]["problem_set_content"].append(
                {"sort_key": sort_key, "extra_content": latex_to_html(extra_content)}
            )
...
def api_exposure():
    ...
    if json["action"] == "delete_exposure":
        ...
        exposure_id = json.get("exposure_id")
        db_data = (
            db.session.query(User.name_last, User.name_first, User.id, ProblemSet.id, ProblemSet.is_adhoc)
            .filter(
        ...
        for user_name_last, user_name_first, user_id, problem_set_id, problem_set_is_adhoc in db_data:
            ...
            problem_ids = map(
                ...
                .filter(ProblemSetContent.problem_set_id == problem_set_id)
                .order_by(ProblemSetContent.sort_key)
                .all(),
            )
        ...
        exposure_record = ExposureContent.query.filter_by(
            exposure_id=exposure_id, user_id=user_id, problem_set_id=problem_set_id
        ).first()
        ...
        if (
            problem_set.is_adhoc
            and not db.session.query(ExposureContent.exposure_id)
            .filter(ExposureContent.problem_set_id == problem_set_id)
            .first()
        ):
        ...
        existing_grading_results = ExposureGradingResult.query.filter_by(
            exposure_grading_id=grading.id, user_id=user_id, problem_set_id=problem_set_id
        ).all()
                ...
                if problem_status_id >= 0:
                    if problem_id in existing_grading_results:
                        ...
                        grading_result = ExposureGradingResult(
                            grading.id, user_id, problem_set_id, problem_id, problem_status_id
                        )
...
@exposures_blueprint.route("/api/exposure_bulk", methods=["POST"])
@flask_login.login_required
def api_exposure_bulk():
    json = request.get_json()
    if not json or "action" not in json or "course_id" not in json or "exposure_id" not in json:
        return jsonify(error="Expected JSON format, action, course_id and exposure_id")

    action = json["action"]
    course_id = json["course_id"]
    exposure_id = json["exposure_id"]
    ...
    if action == "send_for_revision":
        db_data = (
            db.session.query(User.id, Problem.id)
            .filter(
                ExposureContent.user_id == User.id,
                ExposureContent.exposure_id == exposure_id,
                ProblemSetContent.problem_set_id == ExposureContent.problem_set_id,
        ...
        problem_status_id = ProblemStatusInfo.query.filter_by(code="SOLUTION_NEEDS_REVISION").id

        for user_id, problem_id in db_data:
            ps = ProblemStatus.query.filter_by(user_id=user_id, problem_id=problem_id).first()
            if ps:
                if ps.problem_status_id < problem_status_id:
            ...
            if not History.query.filter_by(
                problem_id=problem_id, user_id=user_id, event="SENT_FOR_REVISION_DURING_EXPOSURE_GRADING"
            ).first():
                h = History()
    ...
    elif action == "set_revision_time_interval":
        revision_time_interval = int(json["revision_time_interval"])
        db_data = (
            db.session.query(User.id, Problem.id)
            .filter(
                ExposureContent.user_id == User.id,
        ...
        for uid, pid in db_data:
            user_problems[uid].add(pid)

        for user_id in user_problems:
            ed = ExtraData.query.filter_by(user_id=user_id, course_id=course_id, key="revision_time_interval").first()
            if ed:
                deadlines = flask_json.loads(ed.value)
                for pid in user_problems[user_id]:
                    deadlines[str(pid)] = revision_time_interval
                ed.value = flask_json.dumps(deadlines)
            else:
                ed = ExtraData()
                ed.user_id = user_id
...
def view_exposure_table(course_id, exposure_date):
    ...
    exposure_ids = list(
        map(attrgetter("id"), filter(lambda x: x.timestamp.isoformat()[:10] == exposure_date, exposures))
    )
...
def new_exposure():
    ...
    exposure_venue = request.form.get("exposure_venue", "class")
    ...
    if exposure_type == "manual":
        ...
        exposure.is_autogenerated = False
        exposure.extra_metadata = flask_json.dumps({"exposure_venue": exposure_venue})
        ...
        return redirect(
            url_for("exposures.view_exposure", exposure_string=str(exposure.id), course_id=course_id, group=None)
        )

    if exposure_type == "auto":
        user_topic_estimated_complexity = dict()
        for user_id, topic_id, event in (
            db.session.query(History.user_id, History.comment, History.event)
        ...
        user_trajectories = (
            db.session.query(User.id, Trajectory.id)
            .filter(
        ...
        for _, trajectory_id in user_trajectories:
            if trajectory_id not in trajectory_contents_dict:
                trajectory_topic_ids = [
                    x[0]
                    for x in db.session.query(TrajectoryContent.topic_id)
                    .filter(TrajectoryContent.trajectory_id == trajectory_id)
                    .order_by(TrajectoryContent.sort_key)
                    .all()
                ]
                trajectory_contents_dict[trajectory_id] = trajectory_topic_ids

        topic_ids = reduce(operator_or, map(set, trajectory_contents_dict.values()), set())

        topic_levels = dict(
            db.session.query(TopicLevelAssignment.topic_id, TopicLevelAssignment.level)
            .filter(TopicLevelAssignment.course_id == course_id, TopicLevelAssignment.topic_id.in_(topic_ids))
            .all()
        )

        problem_ids = set()
        problems_by_topic = defaultdict(set)
        for pta in ProblemTopicAssignment.query.filter(ProblemTopicAssignment.topic_id.in_(topic_ids)).all():
            problems_by_topic[pta.topic_id].add(pta.problem_id)
            problem_ids.add(pta.problem_id)

        clones = defaultdict(set)
        for p1, p2 in db.session.query(ProblemRelation.from_id, ProblemRelation.to_id).filter(
        ...
        for problem_id in clones:
            while True:
                expanded_clone_ids = reduce(
                    operator_or, (clones[clone_id] for clone_id in clones[problem_id]), clones[problem_id]
                )
        ...
        problems_count = {
            p_id: p_count
            for p_id, p_count in db.session.query(ProblemStatus.problem_id, db.func.count(ProblemStatus.user_id))
            .filter(
            ...
            .group_by(ProblemStatus.problem_id)
            .all()
        }
        ...
        for user_id, trajectory_id in user_trajectories:
            user_problems_solved = set(
                x[0]
                for x in db.session.query(ProblemStatus.problem_id)
                .filter(
            ...
            user_problems_solved_clones = (
                reduce(operator_or, (clones[problem_id] for problem_id in user_problems_solved), set())
            ...
            problems_available_for_user = (
                reduce(
                    operator_or, (problems_by_topic[topic_id] for topic_id in user_remaining_trajectory_topics), set()
                )
                - user_problems_solved
            )
            ...
            lp += (
                sum(lp_vars_user_problem[(user_id, problem_id)] for problem_id in problems_available_for_user)
                <= max_problems_per_user
            )

            min_problems = min(
                max_problems_per_user, len(problems_available_for_user), len(user_remaining_trajectory_topics)
            )
            ...
            for topic_id in set(user_remaining_trajectory_topics):
                num_occurrences_of_topic_in_remaining_trajectory[topic_id] = sum(
                    1 for i in user_remaining_trajectory_topics if i == topic_id
                )

                lp_vars_user_topic[(user_id, topic_id)] = LpVariable("user_topic_{}_{}".format(user_id, topic_id))
            ...
            for clone_set in family_of_clone_sets:
                lp_clone_variable = LpVariable("user_clones_{}_{}".format(user_id, next(iter(clone_set))), lowBound=0)
                lp += (
                    sum(lp_vars_user_problem[(user_id, problem_id)] for problem_id in clone_set)
                    <= 1 + lp_clone_variable
                )
            ...
            for problem_id in problems_available_for_user:
                if problem_id in user_problems_seen:
                    lp_objective += (
                        penalty_for_having_same_problem_on_next_quiz * lp_vars_user_problem[(user_id, problem_id)]
                    )
                if problems_count.get(problem_id, 0) > 2:
                    lp_objective += (
                        penalty_for_not_giving_a_brand_new_problem * lp_vars_user_problem[(user_id, problem_id)]
                    )

            user_problems_seen_clones = (
                reduce(operator_or, (clones[problem_id] for problem_id in user_problems_seen), set())
                - user_problems_seen
                - user_problems_solved
            )
            ...
            for problem_id in problems_available_for_user & user_problems_seen_clones:
                lp_objective += (
                    penalty_for_giving_clone_of_exposed_problem * lp_vars_user_problem[(user_id, problem_id)]
                )
            ...
            for i, level in enumerate(active_levels):
                topics_of_current_level = set(user_remaining_trajectory_topics) & set(
                    topic_id for topic_id, topic_level in topic_levels_adjusted.items() if topic_level == level
                )

                lp_vars_level_fulfilled[(user_id, level)] = LpVariable(
                    "user_level_fulfilled_{}_{}".format(user_id, level), cat="Binary"
                )
        ...
        try:
            ...
            log_info = "Problem status is “{}” with objective function value “{}”".format(
                LpStatus[lp.status], lp.objective.value()
            )
        except:  # noqa
        ...
        exposure.title = exposure_title
        exposure.timestamp = datetime.strptime(exposure_timestamp, "%Y-%m-%dT%H:%M")
        exposure.is_autogenerated = True
        exposure.extra_metadata = flask_json.dumps({"exposure_venue": exposure_venue})
        db.session.add(exposure)
        db.session.flush()
...
def view_exposures(course_id):
    ...
    for exposure in Exposure.query.filter(Exposure.course_id == course_id).order_by(Exposure.timestamp.desc()).all():
        exposures.append({"id": exposure.id, "date": exposure.timestamp.strftime("%Y-%m-%d"), "title": exposure.title})
    return render_template("view_exposures.html", exposures=exposures, course_id=course_id)
...
def mark_topic_priority_for_learner(course_id):
    ...
    if not ((user_id == flask_login.current_user.id and role == "LEARNER") or role in ["ADMIN", "INSTRUCTOR"]):
        abort(403)
