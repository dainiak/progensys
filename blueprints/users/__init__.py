...
def api_user_management():
    ...
    if json and "action" in json:
        if json["action"] == "load":
            ...
            db.session.query(ExposureContent).filter(ExposureContent.user_id == user.id).delete()

            db.session.query(Participant).filter(Participant.user_id == user.id).delete()

            db.session.query(GroupMembership).filter(GroupMembership.user_id == user.id).delete()

            db.session.query(ProblemStatus).filter(ProblemStatus.user_id == user.id).delete()
            ...
            msg = Message(
                subject="Временный пароль к информационной системе",
...
def api_authorization():
    if request.form and request.form.get("username") and request.form.get("password"):
        ...
        if (
            ...
            and md5(request.form.get("password")) == user.password_hash
        ):
    ...
    if json.get("action") == "login" and json.get("username"):
        ...
        return jsonify({"id": 0, "username": None}), http_status_codes.HTTP_200_OK
