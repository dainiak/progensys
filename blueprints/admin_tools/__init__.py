    ...
    if request.method == "GET":
        ...
        if user_request == "add_users":
            ...
            if group_code == "CLEAR":
                for username in usernames:
                    user_id = User.query.filter_by(username=username).first().id
                    db.session.query(GroupMembership).filter(GroupMembership.user_id == user_id).delete()
                db.session.commit()
            ...
            for user in User.query.filter(User.username.in_(usernames)).all():
                ...
                msg = Message(
                    subject="Пароль к информационной системе",
                    body=f"""Ваше имя пользователя для входа в систему progensys.dainiak.com: “{user.username}”
...
Письмо выслано автоматически; пожалуйста, не отвечайте на него.""",
            ...
            for (user_id,) in (
                ...
                .filter(Participant.course_id == course_id, Participant.role_id == Role.id, Role.code == "LEARNER")
                .all()
            ):
                ed = ExtraData()
                ed.user_id = user_id
            ...
            for line in data.strip().splitlines():
                ...
                if (
                    ("@" not in email_main)
                    or ("@" not in email_overleaf)
                    or ("@" in group_code)
                    or (not id_stepik.isdecimal())
                ):
                    break
