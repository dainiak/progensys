...
def view_problems(problem_id=None):
    (role_code, course_id) = (
        db.session.query(Role.code, Participant.course_id)
        .filter(Participant.user_id == flask_login.current_user.id, Role.id == Participant.role_id)
        .first()
    )
...
def api():
    ...
    if action == "load":
        query = db.session.query(Problem.id, Problem.statement)
                ...
                "data": list(
                    {
                        ...
                        "topic_codes": "\n".join(
                            x[0]
                            for x in db.session.query(Topic.code)
                            .filter(
        ...
        db.session.query(ProblemTopicAssignment).filter(ProblemTopicAssignment.problem_id == problem.id).delete()
        ...
        if item.get("topic_codes") is not None:
            ...
            topic_ids = list(map(lambda x: x[0], db.session.query(Topic.id).filter(Topic.code.in_(topic_codes)).all()))
            db.session.query(ProblemTopicAssignment).filter(ProblemTopicAssignment.problem_id == problem_id).delete()
        ...
        return jsonify(
            {
                ...
                "topic_codes": "\n".join(
                    r[0]
                    for r in db.session.query(Topic.code).filter(
                        Topic.id == ProblemTopicAssignment.topic_id, ProblemTopicAssignment.problem_id == problem_id
                    )
                ),
            }
        )
        ...
        return jsonify(
            {
                ...
                "topic_codes": "\n".join(
                    r[0]
                    for r in db.session.query(Topic.code).filter(
                        Topic.id == ProblemTopicAssignment.topic_id, ProblemTopicAssignment.problem_id == problem.id
                    )
                ),
            }
        )
