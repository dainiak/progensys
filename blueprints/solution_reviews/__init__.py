...
def view_solution_review_requests(course_id):
    ...
    if request.method == "POST":
        ...
        if not (
            current_user_role in ["ADMIN", "INSTRUCTOR", "GRADER"]
            and action
            in [
                "take_for_review",
                "accept_solution",
        ...
        if json.get("action") == "submit_for_review":
            ...
            datetime_last_sent_for_revision, history_event = (
                db.session.query(History.datetime, History.event)
                .filter(
            ...
            if hours_left_for_submission < 0:
                time_points_data = ExtraData.query.filter(
                    ExtraData.user_id == user_id, ExtraData.course_id == course_id, ExtraData.key == "time_points"
                ).first()
        ...
        if action == "load_review_history":
            review_history = (
                db.session.query(History.event, History.comment, History.datetime)
                .filter(
            ...
            for event, comment, timestamp in review_history:
                ...
                if comment and comment.strip():
                    ...
                    history.append(f"{timestring}: {descriptions[event]} с комментарием “{comment}”")
                else:
        ...
        if json.get("notify_learner"):
            ...
            if action == "accept_solution":
                notification_success = notify_user(
                    h.user_id, h.problem_id, f"Решение зачтено с комментарием “{latex_to_html(h.comment)}”"
                )
            elif action == "send_for_revision":
                notification_success = notify_user(
                    h.user_id,
    ...
    review_requests = []
    for user_id, name_first, name_last, problem_id in (
        db.session.query(User.id, User.name_first, User.name_last, ProblemStatus.problem_id)
        .filter(
        ...
        if user_id not in user_groups_cache:
            user_groups_cache[user_id] = " ".join(
                str(x[0])
                for x in db.session.query(Group.code)
                .filter(GroupMembership.user_id == user_id, GroupMembership.group_id == Group.id)
                .all()
            )
    ...
    return render_template(
        "view_solution_review_requests.html", review_requests=to_json_string(review_requests), course_id=course_id
    )
