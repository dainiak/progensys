...
from blueprints.models import db, Topic, Participant, Role, TrajectoryContent, Trajectory, TopicLevelAssignment
...
def api_trajectory():
    ...
    course_id = (
        db.session.query(Participant.course_id)
        .filter(
    ...
    if action == "load":
        data = (
            db.session.query(TrajectoryContent.sort_key, TrajectoryContent.id, TrajectoryContent.topic_id, Topic.code)
        ...
        items = {
            tc.id: tc
            for tc in db.session.query(TrajectoryContent)
            .filter(TrajectoryContent.trajectory_id == trajectory_id, TrajectoryContent.id.in_(new_order))
            .all()
        }
        ...
        if topic_level:
            tla = TopicLevelAssignment.query.filter(
                TopicLevelAssignment.course_id == course_id, TopicLevelAssignment.topic_id == topic_id
            ).first()
            if not tla:
        ...
        return jsonify(
            id=tc.id, topic_id=tc.topic_id, topic_code=topic_code, sort_key=tc.sort_key, topic_level=topic_level
        )
...
def api_trajectories():
    ...
    if action == "load":
        ...
        db.session.commit()
        return jsonify({"id": trajectory.id, "title": trajectory.title, "comment": trajectory.comment})
...
def view_trajectory(trajectory_id):
    ...
    return render_template("view_trajectory.html", trajectory_id=trajectory_id, trajectory_title=trajectory_title)
