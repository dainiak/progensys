...
def api_courses():
    ...
    if action == "get_available_courses":
        data = (
            db.session.query(Course.id, Course.title, Course.description, Role.title, Role.description)
            .filter(
        ...
        role_code = (
            ...
            .filter(Participant.user_id == user_id, Participant.course_id == course_id, Participant.role_id == Role.id)
            .scalar()
        )
        if not role_code:
            abort(403)
        ...
        if role_code in ["ADMIN", "INSTRUCTOR"]:
            action_list.append({"title": "Редактирование задач", "url": url_for("problems.view_problems")})
        if role_code in ["ADMIN", "INSTRUCTOR", "GRADER"]:
            action_list.append(
                {"title": "Создать набор задач…", "url": url_for("problem_sets.new_problem_set", course_id=course_id)}
            )

        if role_code in ["ADMIN", "INSTRUCTOR"]:
            action_list.append({"title": "Редактирование тем", "url": url_for("topics.view_topics")})
        if role_code in ["ADMIN", "INSTRUCTOR"]:
            action_list.append(
                {
...
def view_course(course_id):
    ...
    return render_template("view_course.html", course_id=course_id, course_title=course_title)
...
def api_participants():
    ...
    if action == "load":
        query = db.session.query(
            ...
            Participant.role_id,
            Role.code,
        ).filter(Participant.course_id == course_id, Participant.user_id == User.id, Role.id == Participant.role_id)
        ...
        if json.get("filter"):
            ...
            if frontend_filter.get("groups"):
                ...
                query = query.filter(
                    GroupMembership.user_id == User.id, GroupMembership.group_id == Group.id, Group.code.in_(groups)
                )
        ...
        for uid, username, name_first, name_last, name_middle, email, role_id, role_code in query.all():
            groups = (
                db.session.query(Group.code)
                .filter(Group.id == GroupMembership.group_id, GroupMembership.user_id == uid)
                .all()
            )
            ...
            if role_code == "LEARNER":
                trajectory_id = (
                    db.session.query(Trajectory.id)
                    .filter(
        ...
        if (
            db.session.query(Participant.user_id)
            .filter(Participant.user_id == user_id, Participant.course_id == course_id)
            .first()
        ):
            abort(400)
        ...
        db.session.commit()
        uid, username, name_first, name_last, name_middle, email, role_id = (
            db.session.query(
                User.id,
                User.username,
                User.name_first,
        ...
        role_code = (
            db.session.query(Role.code)
            .filter(Role.id == Participant.role_id, Participant.user_id == user_id, Participant.course_id == course_id)
            .first()
        )
        if role_code:
        ...
        data = ExtraData.query.filter(
            ExtraData.user_id == json["user_id"], ExtraData.course_id == course_id, ExtraData.key == json["key"]
        ).first()
        if data is None:
            data = ExtraData()
