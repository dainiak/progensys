...
def api_grading(json=None):
    ...
    if json["action"] == "load":
        ...
        for (
            ...
            graded_problems = (
                db.session.query(ExposureGradingResult.problem_id, ExposureGradingResult.problem_status_id)
                .filter(
                ...
                .order_by(ExposureGrading.timestamp)
                .all()
            )
        ...
        for field_name in item:
            if field_name.startswith("pid"):
                ...
                if problem_status_id > 0:
                    ...
                    if (
                        ProblemStatusInfo.query.filter_by(id=problem_status_id).first().code
                        ...
                        if not History.query.filter_by(
                            ...
                            problem_id=problem_id, user_id=user_id, event="SOLUTION_ACCEPTED"
                        ).first():
...
def view_grading_table(exposure_string, course_id, group=None):
    ...
    user_query = db.session.query(User.id).filter(
        ExposureContent.exposure_id.in_(exposure_ids), ExposureContent.user_id == User.id
    )

    if group is not None:
        group_id = Group.query.filter_by(code=group).first().id
        user_query = user_query.filter(GroupMembership.group_id == group_id, GroupMembership.user_id == User.id)
    ...
    max_problems_per_set_query = (
        db.session.query(ProblemSetContent.problem_set_id, db.func.count(sql_distinct(ProblemSetContent.problem_id)))
    ...
    if group is not None:
        max_problems_per_set_query = max_problems_per_set_query.filter(
            GroupMembership.group_id == group_id, GroupMembership.user_id == User.id, ExposureContent.user_id == User.id
        )
