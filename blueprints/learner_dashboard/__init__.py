...
import flask_login
from flask import Blueprint, render_template, abort, json as flask_json
...
def view_learner_dashboard(course_id, user_id=None):
    ...
    if (flask_login.current_user.id != user_id and role not in ["ADMIN", "INSTRUCTOR"]) or (
        flask_login.current_user.id == user_id and role != "LEARNER"
    ):
        abort(403)
    ...
    exposures = []
    for exposure_id, exposure_timestamp, problem_set_id in (
        db.session.query(Exposure.id, Exposure.timestamp, ExposureContent.problem_set_id)
        .filter(
            ExposureContent.exposure_id == Exposure.id,
            Exposure.course_id == course_id,
            ExposureContent.user_id == user_id,
        )
        .order_by(Exposure.timestamp)
        .all()
    ):
        problem_set_grading_results = (
            db.session.query(ProblemSetContent.problem_id, ProblemStatusInfo.icon)
            .filter(
            ...
            .order_by(ExposureGrading.timestamp.desc())
            .all()
        )
        ...
        for problem_id, icon in problem_set_grading_results:
            if problem_id in newer_results:
                continue
            grading_results.append({"problem_id": problem_id, "icon": icon})
    ...
    trajectory_id = (
        db.session.query(UserCourseTrajectory.trajectory_id)
        .filter(UserCourseTrajectory.user_id == user_id, UserCourseTrajectory.course_id == course_id)
        .scalar()
    )
    ...
    for problem_id, code in (
        db.session.query(ProblemStatus.problem_id, ProblemStatusInfo.code)
        .filter(
    ...
    topic_levels = dict(
        ...
        .filter(TopicLevelAssignment.course_id == course_id)
        .all()
    )

    trajectory = []
    for topic_id, topic_title, topic_code in (
        db.session.query(Topic.id, Topic.title, Topic.code)
        .filter(TrajectoryContent.trajectory_id == trajectory_id, Topic.id == TrajectoryContent.topic_id)
    ...
    final_grade = get_final_grade(num_topics_per_level, num_checked_topics_per_level)
    final_grade_with_revision = get_final_grade(num_topics_per_level, num_checked_topics_per_level_with_revision)
    # final_grade = final_grade_with_revision = '—'

    revisions = []
    for (problem_id,) in (
        db.session.query(ProblemStatus.problem_id)
        .filter(
            ProblemStatus.user_id == user_id,
            ProblemStatus.status_id == ProblemStatusInfo.id,
        ...
        user_problem_history = (
            History.query.filter(
                ...
                History.event.in_(
                    ["SENT_FOR_REVISION_DURING_EXPOSURE_GRADING", "SENT_FOR_REVISION", "LEARNER_SENT_REVIEW_REQUEST"]
                ),
            )
            .order_by(History.datetime.desc())
            .first()
        )
    ...
    sharelatex_project_id = (
        db.session.query(ExtraData.value)
        .filter(
            ExtraData.user_id == user_id, ExtraData.course_id == course_id, ExtraData.key == "sharelatex_project_id"
        )
        .first()
    )
    ...
    time_points = (
        db.session.query(ExtraData.value)
        .filter(ExtraData.user_id == user_id, ExtraData.course_id == course_id, ExtraData.key == "time_points")
        .first()
    )
    time_points = time_points and time_points[0]

    gdrive_scans_folder_id = (
        db.session.query(ExtraData.value)
        .filter(
            ExtraData.user_id == user_id, ExtraData.course_id == course_id, ExtraData.key == "gdrive_scans_folder_id"
        )
        .first()
    )
