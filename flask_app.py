...
def create_test_lp(group_number):
    ...
    for user_id in users:
        for problem_id in problems_available_for_user[user_id]:
            lp_variables[(user_id, problem_id)] = LpVariable(name = 'assign_{}_{}'.format(user_id, problem_id), cat='Binary')

    for user_id in users:
        # Number of problems for user should definitely not exceed the maximum
        lp += (sum(lp_variables[(user_id, problem_id)] for problem_id in problems_available_for_user[user_id]) <= max_problems)
        # Number of problems for user should preferably not be less than minimum
        min_problems = min(max_problems, len(problems_available_for_user[user_id]), len(remaining_trajectory_items))
        lp_underflow_variable = LpVariable('underflow_{}'.format(user_id), lowBound = 0)
        lp += (sum(lp_variables[(user_id, problem_id)] for problem_id in problems_available_for_user[user_id]) + lp_underflow_variable >= min_problems)
    ...
    for user_id in users:
        for topic_id in remaining_trajectory_items[user_id]:
            ...
            if len(problems_for_user_and_topic) >= 2:
                # The number of problems for any topic should not exceed the number of times this topic appears in a trajectory
                # TODO: make it work when the number of times exceeds 2
                lp += (sum(lp_variables[(user_id, problem_id)] for problem_id in problems_for_user_and_topic ) <= 1)

    for user_id in users:
        for problem_id in problems_available_for_user[user_id]:
            if problem_id in exposed_problems[user_id]:
                lp_objective += penalty_for_having_same_problem_on_next_quiz * lp_variables[(user_id, problem_id)]
            if num_appearances[problem_id] > 0:
                lp_objective += penalty_for_not_giving_a_brand_new_problem * lp_variables[(user_id, problem_id)]
        clones_of_exposed_problems = functools.reduce(operator.or_, (clones[problem_id] for problem_id in exposed_problems[user_id]), set()) - exposed_problems[user_id]
        for problem_id in problems_available_for_user[user_id] & clones_of_exposed_problems:
            lp_objective += penalty_for_giving_clone_of_exposed_problem * lp_variables[(user_id, problem_id)]

    lp.setObjective(lp_objective)

    try:
        lp.solve()
        log_info = 'Problem status is “{}” with objective function value “{}”'.format(LpStatus[lp.status], lp.objective.value())
    except:
        log_info = 'Failed to solve the linear program'

    problem_sets = defaultdict(list)
    for user_id, problem_id in lp_variables:
        if lp_variables[(user_id, problem_id)].value() > 0:
